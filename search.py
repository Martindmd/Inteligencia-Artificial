...
class CornerReversiState:
    ...
    def isGoal(self, min_corners=1):
        ...
        corners = [
            self.board.get((1, 1)),
            self.board.get((1, self.height)),
            self.board.get((self.width, 1)),
            self.board.get((self.width, self.height)),
        ]
    ...
    def result(self, move):
        ...
        # flip enemy
        for enemy in enemy_captured_by_move(
            self.board,
            move,
            self.cur_player,
            adversary,
    ...
    def __getAsciiString(self):
        ...
        for y in range(self.height + 1):
            ...
            for x in range(self.width + 1):
                if x > 0 and y > 0:
                    if (x, y) in moves:
                        rowLine = rowLine + self.board.get(
                            (x, y),
                            "_",
                        )
                    else:
...
def createRandomReversiGeneralState(moves, h, w):
    puzzle = CornerReversiState(
        create_standard_board(h, w, "B", "W"), height=h, width=w
    )
...
if __name__ == "__main__":
    ...
    for method in ["DFS", "BFS"]:
        ...
        if path:
            ...
            print(
                "%s found a path of %d moves visiting %d nodes in %s: %s"
                % (
                    method,
                    len(path),
                    visited,
            ...
            if print_steps:
                ...
                for a in path:
                    ...
                    print(
                        "After %d move%s by %s: %s"
                        % (i, ("", "s")[i > 1], cur_player, print_a)
                    )
                    print(curr)
        else:
            print(
                "%s found no path in %s" % (method, str(timedelta(seconds=end - start)))
            )
